...
| 使用场景         | 1. 组件需要存储一些值，但不影响渲染逻辑。<br>2. 常用于访问 DOM 元素<br>a. 存储 timeout ID<br>b. 存储和操作 DOM 元素，如：通常用于非破坏性操作，例如聚焦、滚动或测量 DOM 元素<br>c. 存储不需要被用来计算 JSX 的其他对象 | 适用于需要触发组件更新和渲染的场景 |
