...
2. 标签必须闭合
3. 使用驼峰式命名法给大部分属性命名 ！React DOM元素中所有对应的属性（https://zh-hans.react.dev/reference/react-dom/components/common）
...
2. 与 state 的区别
-----------------------ref------------------------|--------------------------state---------------------
useRef(initialValue)返回 { current: initialValue }| useState(initialValue) 返回 ([变量的当前值,设置函数])
更改时不会触发重新渲染                                | 更改时触发重新渲染
可变 -- 你可以在渲染过程之外修改和更新 current 的值      | 不可变 -- 必须使用 state 设置函数来修改 state 变量，从而排队重新渲染
你不应在渲染期间读取（或写入）current 值                | 你可以随时读取 state。但是，每次渲染都有自己不变的 state 快照

3. 使用场景：1.组件需要存储一些值，但不影响渲染逻辑。2.常用于访问 DOM 元素
    a. 存储 timeout ID
    b. 存储和操作 DOM 元素，如：通常用于非破坏性操作，例如聚焦、滚动或测量 DOM 元素
    c. 存储不需要被用来计算 JSX 的其他对象。
